...
console = Console()
app = typer.Typer(help="Workspace command line client.", invoke_without_command=True)
remote_app = typer.Typer(help="Manage Workspace remotes.")
agent_app = typer.Typer(help="Run and manage Workspace agents.")
...
def _print_remote_context(store: ConfigStore, remote_name: str | None) -> None:
    ...
    console.print(f"Workspace CLI {__version__}")
...
def chat(
    ...
    agent_slug: str = typer.Option(
        "default-chatbot", "--agent-slug", help="Workspace agent slug."
    ),
...
def eval_agent(
    ...
    agent_slug: str = typer.Option(..., "--agent-slug", help="Workspace agent slug."),
    ...
    timeout_seconds: float = typer.Option(900, "--timeout-seconds", help="Per item Workspace API timeout."),
):
...
def show_agent(
    agent_slug: str = typer.Argument(..., help="Workspace agent slug."),
    remote: str | None = typer.Option(None, "--remote", help="Remote name."),
    as_json: bool = typer.Option(False, "--json", help="Output raw JSON."),
):
