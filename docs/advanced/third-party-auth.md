...
平台 可以通过 OpenID Connect（OIDC）接入企业身份提供商。功能默认关闭；开启前，需要在身份提供商中注册客户端，并准备一个与 平台 地址完全一致的回调地址。
...
把下面的后端回调地址注册到允许的 Redirect URI。生产环境请替换为实际的 平台 域名并使用 HTTPS：

```text
https://<your-yuxi-host>/api/auth/oidc/callback
```

本机开发可以使用 Vite 代理后的 `http://localhost:5173/api/auth/oidc/callback`，前提是身份提供商允许该回调地址；不要在不受信任的网络中使用 HTTP。平台 部署在反向代理后时，回调地址必须是用户实际访问的外部地址，而不是容器内部地址。
...
没有配置 `OIDC_AUTHORIZATION_ENDPOINT` 时，平台 会根据 `OIDC_ISSUER_URL` 请求 `/.well-known/openid-configuration`，读取授权、换 token 和 UserInfo 端点。只要填写了 `OIDC_AUTHORIZATION_ENDPOINT`，代码就把手动端点当作权威配置并跳过 discovery；这时 `OIDC_TOKEN_ENDPOINT` 和 `OIDC_USERINFO_ENDPOINT` 也要按实际流程补齐，否则回调换 token 或读取用户信息会失败。
...
| 变量 | 默认值 | 作用 |
| --- | --- | --- |
| `OIDC_USE_RAW_USERNAME` | `false` | 是否用 OIDC 用户名作为 平台 的 `uid` |
...
5. 前端使用 code 调用 `/api/auth/oidc/exchange-code`，再进入 平台。

## 使用原始用户名绑定已有账号

将 `OIDC_USE_RAW_USERNAME=true` 后，平台 会尝试用 OIDC 返回的用户名匹配已有的 `uid`。匹配成功后，系统会创建一条已删除状态的占位用户记录，保存 `OIDC sub` 与目标用户的绑定关系；占位记录不能用于登录。
...
如果同一个 OIDC `sub` 已绑定到其他用户，登录会被拒绝，防止因为用户名相同而接管账号。启用此模式前，请确认 OIDC 用户名在身份提供商中稳定且唯一，并提前创建需要绑定的 平台 用户。
