...
平台 的品牌配置分为两部分：后端读取的站点信息，以及前端源码中的主题样式。前者可以通过 YAML 文件替换名称、Logo 和协议链接；后者需要修改前端资源。

默认站点名称为“知识工作台”，图标使用通用窗口图形，版权页脚为空。首页、登录页和侧栏没有原项目推广入口；Web 内的帮助入口读取随项目交付的站内说明。Web 的 build、dev 和 server 命令从当前 `docs/` 生成 `/help/`，该目录是派生产物；更新文档后重新构建或重启开发服务器即可同步。已有部署的自定义 YAML 和已保存的智能体配置属于部署数据，需要由部署者核对。
