...
平台 通过 `KnowledgeBaseManager` 读取知识库配置、解析权限并选择 executor：

- `milvus` 是文档型知识库，支持上传、解析、分块、向量索引、预览和检索，也可以构建知识图谱；
- `dify` 和 `notion` 是只读连接器，只保存外部连接信息并执行 Query，不承载 平台 的上传、解析、索引、文件树和全文预览。
...
- `backend/server/routers/knowledge_router.py`：权限、上传、任务和状态筛选
- `backend/package/yuxi/knowledge/manager.py`：配置回源、可见性和 executor 调度
- `backend/package/yuxi/knowledge/base.py`：文件状态和解析流程
- `backend/package/yuxi/knowledge/implementations/milvus.py`：分块、双写、检索和重索引
- `backend/package/yuxi/knowledge/implementations/read_only_connectors.py`：Dify/Notion 能力边界
- `backend/package/yuxi/services/task_service.py`：任务持久化、claim、lease 和恢复结局
- `backend/package/yuxi/services/task_registry.py`：领域 Handler 注册和惰性加载
- `backend/package/yuxi/agents/toolkits/kbs/tools.py`：Agent 目标校验和工具实现
- 知识库 unit tests（`backend/test/unit/knowledge`）
- 权限与路由 tests（`backend/test/unit/routers`）
- `backend/test/integration/api/test_knowledge_router.py`
- `backend/test/integration/api/test_knowledge_external_router.py`
