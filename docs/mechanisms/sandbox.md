...
平台 让多个访问入口看到同一份持久文件，但给它们不同的访问能力：
...
- `backend/package/yuxi/agents/backends/sandbox/provider.py`：runtime identity、缓存和 keepalive
- `backend/package/yuxi/workspace/paths.py`：uid 与 Workdir 映射
- `backend/package/yuxi/workspace/filesystem.py`：宿主 no-follow 文件原语
- `docker/sandbox_provisioner/app.py`：Docker/Kubernetes 创建、代理和回收
- `backend/package/yuxi/storage_migration.py`：历史布局迁移
- Sandbox backend unit tests（`backend/test/unit/backends`）
- Workspace/Workdir unit tests（`backend/test/unit/workspace`）
- `backend/test/integration/services/test_project_workdir_provisioner.py`
