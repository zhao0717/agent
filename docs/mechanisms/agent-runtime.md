...
本页解释 平台 如何从持久化 Agent 配置构建一次运行，以及配置、权限、文件和 LangGraph state 在运行中分别负责什么。Agent 配置和扩展开发见[配置和开发智能体](../agents/agents-config.md)。
...
- `backend/package/yuxi/agents/middlewares/memory.py`
- 运行时上下文 unit（`backend/test/unit/agents`）
- Agent 主链路 E2E（`backend/test/e2e`）
