...
长对话和大段工具结果会占用模型上下文。平台 使用一个压力阈值控制自动压缩：请求达到阈值后先确定性压缩大工具结果，重新计量后仍达到同一阈值才调用摘要模型。配置入口见[智能体配置](../agents/agents-config.md)，中间件顺序见[中间件](../agents/middleware.md)。
...
入口阈值没有达到时，平台 直接调用主模型。请求达到阈值后，确定性压缩先执行；压缩后的请求低于同一阈值时不调用摘要模型。主模型返回 `ContextOverflowError` 时，系统把它视为强制摘要信号。
...
- `backend/package/yuxi/services/context_compression_service.py`
- `backend/package/yuxi/repositories/agent_state_repository.py`
...
- Summary unit tests（`backend/test/unit/middlewares`）
- `backend/test/unit/services/test_context_compression_service.py`
- `backend/test/integration/services/test_summary_middleware_real_model.py`
