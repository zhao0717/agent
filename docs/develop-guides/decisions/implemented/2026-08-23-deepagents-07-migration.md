...
平台 此前锁定 `deepagents==0.6.7` 与旧版 LangChain 底座（langchain 1.3.10、langchain-core 1.4.8）。升级到 DeepAgents 0.7 存在以下硬性断点：
...
2. **`CompositeBackend` 路由补丁**：0.6 时 平台 维护 `CustomCompositeBackend` 修复 route-aware glob 逻辑；0.7 官方已修复此问题并支持 `GlobResult.truncated`。
...
- **整体切换为 `create_deep_agent`**：放弃 平台 自定义 AgentRun、持久化子智能体、审批、Steer、Skills 链路，改用上游全家桶。被拒绝：风险远超依赖升级本身，破坏 平台 核心调度与持久化事实。
- **保留全局可变 backend factory**：违背 0.7 契约，且跨 Run/跨用户存在竞态风险。被拒绝。

## 后果

- 删除了 平台 自行维护的 `CustomCompositeBackend`。
