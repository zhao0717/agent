...
管理接口只接受 `streamable_http` 和 `sse`。平台 不允许通过 HTTP 请求创建任意本地 `stdio` 进程；历史用户 `stdio` 配置会被禁用，应迁移为远程服务。
...
开发者在 `backend/package/yuxi/agents/mcp/service.py` 的 `_DEFAULT_MCP_SERVERS` 中添加固定定义：
