...
智能体评估用 Langfuse Dataset 保存一组固定任务，再让 平台 按真实的 AgentRun、worker 和工具链路逐条执行。它适合比较一个智能体在研究、编程、文件处理或多步骤任务上的表现。

本页不介绍知识库的 `recall@K` 和答案指标；那部分见[知识库评估](../intro/evaluation.md)。

## 需要准备什么

1. 平台 已配置 Langfuse tracing，API/worker 环境包含：
...
平台 不负责创建或上传 Dataset。先在 Langfuse 中检查任务文本、参考输出和数据集版本，再运行实验。
...
1. 从 Dataset 读取任务文本；
2. 调用 平台 的 `POST /api/agent-invocation/eval/runs`；
3. 由 平台 创建临时 Conversation 和 AgentRun；
4. 通过 worker 执行真实智能体；
5. 等待 Run 进入终态；
6. 把最终输出写回 Langfuse experiment item。

`--max-concurrency` 是 Dataset 实验的并发数。先从 `1` 开始，再根据模型服务、worker 和沙盒容量提高。`--timeout-seconds` 是每条样例等待 平台 结果的上限；超时会报告当前运行状态，不应把它当作成功。

## 查看结果

实验完成后，在 Langfuse Dataset 的 experiment 中查看每条 item 的最终输出。平台 会在本地运行上下文和 trace 中保存以下标记，便于筛选：
...
- experiment 有 item 但 平台 失败：检查 CLI 登录的 API Key、Agent slug，并用 `docker compose logs api worker` 查看当前槽位日志。
- Trace 缺失：检查 API/worker 是否读取到 Langfuse 配置；平台 业务结果仍以 PostgreSQL 的 Run 和消息为准。
...
实现入口见 `backend/server/routers/agent_invocation_eval_router.py`、`packages/yuxi-cli/src/yuxi_cli/agent_eval.py` 和 [Langfuse 集成](../advanced/langfuse-integration.md)。
