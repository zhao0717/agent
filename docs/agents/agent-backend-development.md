...
本页面向需要在 平台 中新增或维护 Agent 后端的贡献者。它只讲代码装配；配置字段、权限和运行时上下文分别见[配置智能体](./agents-config.md)和[Agent 运行时上下文](../mechanisms/agent-runtime.md)。
...
- Graph 使用 平台 的模型、工具、文件和 checkpoint 装配入口；
- 工具副作用在执行处验证用户、路径和资源；
...
- `backend/package/yuxi/agents/base.py`
- `backend/package/yuxi/agents/context.py`
- `backend/package/yuxi/agents/buildin/chatbot/graph.py`
- `backend/package/yuxi/agents/buildin/__init__.py`
- Agent unit tests（`backend/test/unit/agents`）
- Agent integration/E2E（`backend/test/e2e`）

改变持久配置、权限、模型可见输入、Run 生命周期或文件边界时，先按 [平台 Spec Loop](../develop-guides/spec-loop.md) 建立相应的决策和验证范围。
