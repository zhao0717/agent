...
一次 Agent 运行可能包含多次模型调用、知识库检索、工具执行和文件操作。为了避免同一对话同时修改同一份上下文，平台 把“收到请求”和“开始运行”分成两个阶段，并为每个线程维护 FIFO 队列。
...
实现入口见 `backend/server/routers/agent_router.py`、`backend/package/yuxi/services/agent_request_queue_service.py` 和`backend/package/yuxi/services/run_worker.py`。
