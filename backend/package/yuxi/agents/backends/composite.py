...
# Workspace 在 DeepAgents 内建排除集之上额外豁免知识库文档工具结果，
...
# 文件工具 allowlist：显式排除 destructive delete。Workspace backend 未实现 delete，
...
class _BackendScope:
    ...
    def create_backend(self) -> CompositeBackend:
        ...
        # 派生 large_tool_results 与 conversation_history 前缀，与 Workspace 契约一致。
